...
if __name__ == '__main__':
    folds_N = 5
    Errs = np.zeros((3, folds_N, 3))

    for i in range(1):
        Errs[0, i] = cnn_train(dataset="DB3_mixeduser_2_16_5folds.pkl", saveto="DB3_CNN_mixeduser.npz", max_epoches=100,
                               xdim=6, ydim=62, valid_Freq=220, start_ges=0, end_ges=61, patience=16,
                               fold_id=i)

    # toSave = []
    # for i in range(1):
    #     toSave.append([Errs[i, j, 2] for j in range(5)])
    # np.savetxt("CNN_lower_Errs.txt", np.array(toSave))

    for i in range(3):
        if (i == 0):
            print "DB1_CNN_independent.npz"
        if (i == 1):
            print "DB2_CNN_independent.npz"
        if (i == 2):
            print "DB3_CNN_independent_lower.npz"
